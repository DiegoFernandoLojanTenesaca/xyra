<script lang="ts">
  import { CircleQuestionMark, Download, ExternalLink } from '@lucide/svelte';
...
<section class="card highlight cut">
  ...
  <div class="text">
    ...
    <p class="muted">{t('common:tagline')} · {t('about:license')}</p>
  </div>
  <div class="signatures">
    <small class="muted">{t('about:madeBy')}</small>
    {#each CREATORS as creator (creator.user)}
      <button onclick={() => openExternal(githubProfile(creator.user))} title="@{creator.user}">
        <img class="diamond" src={creator.photo} alt="" />
        <span><b>{creator.name}</b><small class="accent">{creator.team}</small></span>
      </button>
    {/each}
  </div>
</section>
  ...
  <section class="panel cut box">
    ...
    <ul>
      {#each changes as change (change)}
        <li><span class="accent">■</span>{change}</li>
      {/each}
    </ul>
