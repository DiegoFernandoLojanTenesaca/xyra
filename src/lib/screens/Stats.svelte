...
{#if stats && stats.games}
  <div class="tiles">
    <StatTile label={t('stats:games')} value={stats.games} format={format.number} />
    <StatTile label={t('stats:wins')} value={stats.wins} format={format.number} />
    <StatTile label={t('stats:winRate')} value={percent(stats.wins, stats.games)} format={format.percent} accent />
    ...
    {#each stats.recent as game (game.game_id)}
      <div class="game panel" class:won={game.win}>
        ...
        <div class="column">
          ...
          <small class="muted facts"><span>{game.champion.name}</span><span>{t(`common:modes.${game.mode}`)}</span><span>{format.date(game.date)}</span></small>
        </div>
        <div class="augments">
...
<style>
  ...
  .game {
    ...
    gap: var(--space-4);
    padding: var(--space-2) var(--space-4);
    border-left: var(--border-accent) solid var(--color-textFaint);
  }
  .game.won {
