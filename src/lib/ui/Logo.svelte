...
<svg viewBox="0 0 256 256" width={size} height={size} class:live={phase === 'inGame'} class:still={phase === 'paused'} aria-hidden="true">
  <defs>
    ...
    <mask id="{id}-gap"><rect class="shown" width="256" height="256" /><polygon class="hidden" points="128,75 181,128 128,181 75,128" /></mask>
  </defs>
  ...
  <polygon class="core" points="128,92 164,128 128,164 92,128" fill={phase === 'inGame' ? `url(#${id}-red)` : `url(#${id}-gray)`} />
</svg>
...
<style>
  ...
  .live .petals {
    filter: drop-shadow(0 0 var(--effect-glow) color-mix(in srgb, var(--color-accent) 65%, transparent));
  }
  @keyframes spin {
    to {
