...
- **Cambia opciones oficiales del juego** desde la página Juego (sin bordes, rango de ataque, cronómetros del minimapa,
  ...
  (`crates/xyra-core/src/juego.rs`). Los cronómetros son los que trae el propio juego: Xyra no dibuja cronómetros
...
Todo queda en `%APPDATA%\com.indagalab.xyra\`: `config.json`, `stats.json` (tus partidas), `perfil.json` (tu nombre,
nivel y maestrías, para verlos con el LoL cerrado), `catalogo.json` (nombres e íconos), `xyra.log` (registro, se reinicia
al pasar de 1 MB) y `capturas\` (solo si activas "Guardar capturas"). Desde *Ajustes → Datos* puedes exportar tus
