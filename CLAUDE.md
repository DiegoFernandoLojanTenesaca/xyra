...
| Path | Owns |
|---|---|
| `crates/xyra-core/` | Platform-independent logic, reusable by a future mobile companion: card detection and rating (`cards`), OP.GG (`opgg`), the League client over HTTPS and WebSocket (`league`), client state (`gameflow`, and `champ_select` with counter picks, summoner spells and the automatic build import), `catalog`, `stats`, `profile`, `config`, rune, spell and item set import (`client_import`), official game options (`game_settings`), every file on disk (`storage`), i18n, and the design tokens as constants generated by `build.rs` (`theme::tokens`), typed failures (`errors`), the internet client (`web`) and GitHub releases for in-app updates (`updates`). |
...
To see what the card reader reads in a real game, run the probe while playing; it prints every read that finds a card name:
`$env:XYRA_CATALOG="$env:APPDATA\com.indagalab.xyra\catalog.json"; cargo test -p xyra --lib probes_the_live_screen -- --ignored --nocapture`
(`XYRA_PROBE_SECONDS` sets how long it runs).